...
.    if ${OPSYS} == "NetBSD"
SSLDIR=	${PREFIX}/etc/openssl
.    elif ${OPSYS} == "Linux"
.      if exists(${_CROSS_DESTDIR:U}${PREFIX}/etc/pki/tls)
# Some distributions have moved to ${PREFIX}/etc/pki/tls, with incomplete
# symlinks from ${PREFIX}/etc/ssl.  Prefer the new location if it exists
SSLDIR=	${PREFIX}/etc/pki/tls
.      else
SSLDIR=	${PREFIX}/etc/ssl 		# standard location
.      endif
.    elif ${OPSYS} == "Haiku"
...
.      endif
.    else
SSLDIR=	${PREFIX}/etc/ssl 		# most likely place
.    endif
.  else
